...
normalize the variables for feature importance
```{r}
scaled.train <- data.Normalization(train, type = "n12", normalization = "column")
scaled.test <- data.Normalization(test, type = "n12", normalization = "column")
```
...
fit.cv <- cv.glmnet(subset(scaled.train, select = -throughput_mbits), 
...
pred.cv <- predict(object = fit.cv, newx = subset(scaled.test, select = -throughput_mbits), 
...
plot(subset(scaled.test, select = throughput_mbits), pred.cv, main = "CV.GLMNET", 
     xlab = "truth", ylab = "prediction")
```
...
yq <- mean(subset(scaled.test, select = throughput_mbits))
R2 <- sum((pred.cv-yq)^2)/sum((subset(scaled.test, select = throughput_mbits)-yq)^2)
R2

#calculate MSE
n <- 1/length(pred.cv)
mse <- n*sum((subset(scaled.test, select = throughput_mbits)-pred.cv)^2)
mse

#calculate MAE
mae <- n*sum(abs(subset(scaled.test, select = throughput_mbits)-pred.cv))
mae
```
