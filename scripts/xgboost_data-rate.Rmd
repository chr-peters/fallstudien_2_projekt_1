...
learner_ul = make_learner()
```

## Validation Results

```{r}
resampling_result_ul = resample(
  task = task_ul,
  learner = learner_ul,
  resampling = make_outer_resampling(task_ul, drive_ids_train=1:7, drive_ids_test=8:10),
  store_models = TRUE
)
```

```{r}
resampling_result_ul$aggregate(msr("regr.rsq"))
resampling_result_ul$aggregate(msr("regr.mae"))
```

```{r}
predictions_ul = as.data.table(resampling_result_ul$prediction())
ggplot(predictions_ul, aes(x=truth, y=response)) +
  geom_point()
```