...
plot_linklifetime_prediction <- function(data,
                                         provider = c("vodafone", "tmobile", "o2"),
                                         width = 30, height = 20, 
                                         view_plot = FALSE, 
                                         filename = "predictions.png"){
  actual <- data[data["provider"] == provider,
                 c("link_lifetime", 
                   "timestamp",
                   "drive_id", 
                   "scenario")]
  actual$type <- "Beobachtung"
  colnames(actual)[colnames(actual) == "link_lifetime"] <- "value"
  
  vorhersage <- data[data["provider"] == provider,
                     c("prediction_xgboost", 
                       "timestamp",
                       "drive_id", 
                       "scenario")]
  vorhersage$type <- "Vorhersage"
  colnames(vorhersage)[colnames(vorhersage) == "prediction_xgboost"] <- "value"
  
  plot_data <- rbind(actual, vorhersage)
  
  plot <- ggplot(
    plot_data, 
    aes(x = anytime(timestamp), y = value, color = type)
  ) + 
    geom_line(size=1) + 
    facet_wrap(drive_id~scenario, 
               scales = "free", 
               ncol = 4, 
               labeller = label_wrap_gen(multi_line = FALSE)) + 
    ggtitle(paste("Link-Lifetime Vorhersage: ", name_mapping[[provider]], sep = "")) + 
    xlab("Zeit") + 
    ylab("Link-Lifetime in Sekunden") +
    theme_grey(base_size = 14) +
    theme(legend.position="bottom", 
          legend.title = element_blank())
  ...
  ggsave(filename=paste(provider, filename, sep="_"), 
         ...
         path = paste(
           "C:/Users/", 
           Sys.getenv("USERNAME"), 
           "/Documents/GitHub/fallstudien_2_projekt_1/presentation/plots/link_lifetime",
           sep = ""),
         width = width, height = height, 
         units = "cm",
         dpi = 200
...
plot_linklifetime_scatter <- function(data,
                                      width = 30, 
                                      height = 20, 
                                      drive_id_colored = FALSE,
                                      view_plot = FALSE, 
                                      filename = "scatter.png"){
  plot_data <- data[, c("link_lifetime", 
                        "prediction_xgboost", 
                        "provider", 
                        "scenario", 
                        "drive_id")]
