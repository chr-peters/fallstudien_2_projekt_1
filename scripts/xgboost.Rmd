...
```{r}
permutation_ul %>% 
  left_join(importance_ul, by=c("feature"="Feature", "provider")) %>% 
  select(provider, feature, score, Gain) %>% 
  rename(Permutation = score) %>%
  write_csv(str_c(results_dir, "feature_importance_xgboost_ul.csv"))
```
...
```{r}
permutation_dl %>% 
  left_join(importance_dl, by=c("feature"="Feature", "provider")) %>% 
  select(provider, feature, score, Gain) %>% 
  rename(Permutation = score) %>%
  write_csv(str_c(results_dir, "feature_importance_xgboost_dl.csv"))
```
...
permutation_linklifetime %>% 
  left_join(importance_linklifetime, by=c("feature"="Feature", "provider")) %>% 
  select(provider, feature, score, Gain) %>% 
  rename(Permutation = score) %>%
  write_csv(str_c(results_dir, "feature_importance_xgboost_linklifetime.csv"))
```